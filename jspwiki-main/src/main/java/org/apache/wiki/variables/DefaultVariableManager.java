...
public class DefaultVariableManager implements VariableManager {
    ...
    private static final WikiLogger log = WikiLogger.getLogger(DefaultVariableManager.class);

    /**
     * Contains a list of those properties that shall never be shown. Put names here
     * in lower case.
     */
     ...
     */
    @Override
    public String parseAndGetValue(final Context context, final String link)
     ...
     */
    @Override
    public String getValue(final Context context, final String varName)
            throws IllegalArgumentException, NoSuchVariableException {
        if (varName == null) {
        ...
        try {
            ...
            //
            // Well, I guess it wasn't a final straw. We also allow variables from the
            // session and the request (in this order).
            //
            ...
            //
            // And the final straw part 2: see if the "real" current page has named
            // metadata. This allows
            ...
            //
            // Next-to-final straw: attempt to fetch using property name. We don't allow
            // fetching any other
            // properties than those starting with "jspwiki.". I know my own code, but I
            // can't vouch for bugs
    ...
    /**
     * This class provides the implementation for the different system variables. It
     * is called via Reflection - any access to a variable called $xxx is mapped to
     * getXxx() on this class.
     * <p>
     * This is a lot neater than using a huge if-else if branching structure that we
     * used to have before.
     * <p>
     ...
     * capitalization anywhere. This may look a bit odd, but then again, this is not
     * meant to be a public class.
     *
     * @since 2.7.0
     */
    ...
    private static class SystemVariables {
        ...
        public String getLoginstatus() {
            ...
            return Preferences.getBundle(m_context, InternationalizationManager.CORE_BUNDLE)
