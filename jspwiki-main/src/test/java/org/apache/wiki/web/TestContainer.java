...
public class TestContainer {
    private static final WikiLogger log = WikiLogger.getLogger(TestContainer.class);
    /**
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if (pair.length != 2) {
                throw new IllegalArgumentException(
                        "Malformed argument '" + args[i] + "'; expected 'context=path' pattern.");
            }
...
//            ContextFactory.setNameParser( new InitialContextFactory.DefaultParser() );
        ...
        try {
            ...
            initCtx.bind(JNDI_ENV_ROOT, new NamingContext(new Hashtable<String, Object>(), JNDI_ENV_ROOT, null,
                    new InitialContextFactory.DefaultParser()));
    ...
    public void addWebApp(final String context, final String path) {
        // Set the default users and roles for the realm (note that realm name *must*
        // match web.xml <realm-name>
    ...
    /**
     * Handler that shuts down the Jetty server if a request is received containing
     * the shutdown string {@link TestContainer#SHUTDOWN_CMD} .
     */
