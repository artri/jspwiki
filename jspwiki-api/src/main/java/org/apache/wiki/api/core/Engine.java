...
public interface Engine {
    ...
    default URL findConfigFile( final String name ) {
        WikiLogger.getLogger( Engine.class ).info( "looking for " + name + " inside WEB-INF " );
        ...
        if ( defaultFile != null && defaultFile.exists() ) {
            try {
                ...
                WikiLogger.getLogger( Engine.class ).warn( "Malformed URL: " + e.getMessage() );
            }
        }
        ...
        if( getServletContext() != null ) {
            ...
            try {
                ...
                WikiLogger.getLogger( Engine.class ).error( "unable to create a temp file to load onto the policy", e );
                return null;
            }
            tmpFile.deleteOnExit();
            WikiLogger.getLogger( Engine.class ).info( "looking for /" + name + " on classpath" );
            ...
            try( final InputStream is = Engine.class.getResourceAsStream( "/" + name );
                ...
                WikiLogger.getLogger( Engine.class ).error( "Your code is b0rked.  You are a bad person.", e );
                throw new WikiRuntimeException(e);
            } catch( final IOException e ) {
                WikiLogger.getLogger( Engine.class ).error( "failed to load security policy from file " + name + ",stacktrace follows", e );
            }
        }
        return path;
    }
